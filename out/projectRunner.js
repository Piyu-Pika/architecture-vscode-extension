...
exports.EnhancedAutoProjectManager = exports.AutoProjectManager = exports.EnhancedProjectRunnerFactory = exports.FastAPIProjectRunner = exports.DjangoProjectRunner = exports.RustProjectRunner = exports.GoProjectRunner = exports.ReactNativeProjectRunner = exports.CMakeProjectRunner = exports.KotlinProjectRunner = exports.NodeProjectRunner = exports.SpringBootProjectRunner = exports.VueProjectRunner = exports.AngularProjectRunner = exports.NextJSProjectRunner = exports.ReactProjectRunner = exports.FlutterProjectRunner = exports.DeviceManager = exports.ProjectDetector = exports.EntryPointDetector = void 0;
...
class EntryPointDetector {
    static async findEntryPoint(projectPath, projectType, projectName) {
    ...
    static async replacePlaceholders(pathTemplate, projectName, projectPath) {
    ...
    static async detectPackagePath(projectPath, projectName) {
    ...
    static async findJavaPackagePath(basePath) {
        try {
    ...
    static async findJavaFiles(dirPath) {
        try {
    ...
    static extractPackageFromPath(fullPath, basePath) {
        const relativePath = path.relative(basePath, fullPath);
        return relativePath.replace(/[\\\/]/g, '/');
    }
    static async findWildcardPath(projectPath, pattern) {
        const parts = pattern.split('*');
        if (parts.length !== 3)
            return null; // Only support one ** pattern
    ...
    static async searchRecursively(basePath, targetPattern) {
        try {
            const items = await fs.promises.readdir(basePath, { withFileTypes: true });
            for (const item of items) {
...
class ProjectDetector {
    ...
    static async calculateRuleScore(projectPath, rule) {
        let score = 0;
    ...
    static async getAllProjectInfo(projectPath) {
...
exports.ReactProjectRunner = ReactProjectRunner;
// NextJSProjectRunner
class NextJSProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.NextJSProjectRunner = NextJSProjectRunner;
class AngularProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.AngularProjectRunner = AngularProjectRunner;
// VueProjectRunner
class VueProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.VueProjectRunner = VueProjectRunner;
// SpringBootProjectRunner
class SpringBootProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.SpringBootProjectRunner = SpringBootProjectRunner;
...
exports.NodeProjectRunner = NodeProjectRunner;
//kotlinProjectRunner
class KotlinProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.KotlinProjectRunner = KotlinProjectRunner;
//cmakeProjectRunner
class CMakeProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.CMakeProjectRunner = CMakeProjectRunner;
...
class GoProjectRunner {
    ...
    async runProject(projectPath, projectName, mode = 'dev') {
        ...
        await dependancyInstall_1.TerminalManager.executeCommands(commands, fullPath);
    }
    async getRunCommandsWithEntryPoint(mode, entryPoint, projectPath) {
...
exports.GoProjectRunner = GoProjectRunner;
class RustProjectRunner {
    async findEntryPoint(projectPath) {
    ...
    async getRunCommandsWithEntryPoint(mode, entryPoint) {
...
class DjangoProjectRunner {
    ...
    async runProject(projectPath, projectName, mode = 'dev') {
        ...
        await dependancyInstall_1.TerminalManager.executeCommands(fullCommands, fullPath);
    }
    async getRunCommandsWithEntryPoint(mode, entryPoint) {
...
exports.DjangoProjectRunner = DjangoProjectRunner;
class FastAPIProjectRunner {
    async findEntryPoint(projectPath) {
...
exports.FastAPIProjectRunner = FastAPIProjectRunner;
...
class EnhancedProjectRunnerFactory {
    static getRunner(projectType) {
        switch (projectType) {
            ...
            case 'Django':
                return new DjangoProjectRunner();
            case 'FastAPI':
                return new FastAPIProjectRunner();
            case 'Rust':
                return new RustProjectRunner();
            case 'ReactNative':
                return new ReactNativeProjectRunner();
            case 'React':
                return new ReactProjectRunner();
            case 'NextJS':
                return new NextJSProjectRunner();
            case 'Angular':
                return new AngularProjectRunner();
            case 'Vue':
                return new VueProjectRunner();
            case 'SpringBoot':
                return new SpringBootProjectRunner();
            case 'Node.js':
                return new NodeProjectRunner();
            case 'CMake':
                return new CMakeProjectRunner();
            case 'Kotlin':
                return new KotlinProjectRunner();
            default:
...
class AutoProjectManager {
    ...
    static async autoRunProject(projectPath, projectName, mode = 'dev') {
        try {
            ...
            if (!detectedType) {
                ...
                if (projectInfo.availableTypes.length > 0) {
                    ...
                    const selectedType = await vscode.window.showQuickPick(projectInfo.availableTypes.map(type => ({ label: type, value: type })), {
    ...
    static async autoInstallAndRun(projectPath, projectName, mode = 'dev') {
        try {
            const fullPath = projectName ? `${projectPath}/${projectName}` : projectPath;
            const detectedType = await ProjectDetector.detectProjectType(fullPath);
            if (!detectedType) {
                vscode.window.showErrorMessage('Could not detect project type for dependency installation.');
                return;
            }
            // Install dependencies first
            const { DependencyInstallerFactory } = await Promise.resolve().then(() => require('./dependancyInstall'));
            const installer = DependencyInstallerFactory.getInstaller(detectedType);
            vscode.window.showInformationMessage(`Detected ${detectedType} project. Installing dependencies...`);
    ...
    static async showProjectInfo(projectPath) {
        try {
    ...
    static async runFromWorkspace(mode = 'dev') {
