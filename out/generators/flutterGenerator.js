...
`);
    ...
    async createBlocArchitectureStructure() {
        const libPath = path.join(this.config.projectPath, 'lib');
    ...
    async createSampleBlocFiles() {
        const blocPath = path.join(this.config.projectPath, 'lib', 'presentation', 'bloc');
        // Create sample counter bloc
        await fs.writeFile(path.join(blocPath, 'counter_event.dart'), `part of 'counter_bloc.dart';
...
`);
        await fs.writeFile(path.join(blocPath, 'counter_state.dart'), `part of 'counter_bloc.dart';
...
`);
        await fs.writeFile(path.join(blocPath, 'counter_bloc.dart'), `import 'package:bloc/bloc.dart';
