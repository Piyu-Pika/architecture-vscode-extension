...
`);
        ...
        await fs.writeFile(coreUrlsPath, `from django.urls import path
from . import views
...
`);
        ...
        await fs.writeFile(viewsPath, `from django.shortcuts import render
...
`);
        ...
        await fs.writeFile(homeTemplatePath, `<!DOCTYPE html>
<html>
<head>
...
`);
        ...
        await fs.writeFile(requirementsPath, `django>=4.2.0\npython-dotenv>=1.0.0\npytest>=7.3.0\npytest-django>=4.5.2\n`);
        // Create manage.py
        const managePath = path.join(this.projectPath, 'manage.py');
        await this.backupIfExists(managePath);
        await fs.writeFile(managePath, `#!/usr/bin/env python
