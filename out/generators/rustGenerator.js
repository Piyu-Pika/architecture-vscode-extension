...
class RustGenerator {
    constructor(projectPath, projectName, framework = 'None/Add later') {
...
`);
            ...
            await fs.writeFile(mainPath, this.getFrameworkMainCode());
        }
        // Create lib.rs
...
`);
        ...
        await fs.writeFile(cargoPath, this.getCargoToml());
        // Create README
...
`);
    ...
    async createActixFiles() {
        ...
        await fs.writeFile(routesModPath, `pub mod health;
pub mod hello;
...
`);
        ...
        await fs.writeFile(healthRoutePath, `use actix_web::{web, HttpResponse, Responder};
use serde::Serialize;
...
`);
        ...
        await fs.writeFile(helloRoutePath, `use actix_web::{web, HttpResponse, Responder};
...
`);
    ...
    async createAxumFiles() {
        ...
        await fs.writeFile(handlersModPath, `pub mod health;
pub mod hello;
`);
        const healthHandlerPath = path.join(handlersDir, 'health.rs');
        await this.backupIfExists(healthHandlerPath);
        await fs.writeFile(healthHandlerPath, `use axum::Json;
use serde::Serialize;
...
`);
        ...
        await fs.writeFile(helloHandlerPath, `use axum::{
    extract::Query,
    Json,
};
use serde::{Deserialize, Serialize};
...
`);
        const healthRoutePath = path.join(routesDir, 'health.rs');
        await this.backupIfExists(healthRoutePath);
        await fs.writeFile(healthRoutePath, `use serde::Serialize;
use warp::Filter;
...
`);
        const helloRoutePath = path.join(routesDir, 'hello.rs');
        await this.backupIfExists(helloRoutePath);
        await fs.writeFile(helloRoutePath, `use serde::{Deserialize, Serialize};
use warp::Filter;
